document.addEventListener('DOMContentLoaded', () => {
    ...
    const filterEvents = () => {
        ...
        const currentLang = localStorage.getItem('language') || 'en'; // جلب اللغة الحالية
        ...
        eventCards.forEach(card => {
            ...
            // قراءة النصوص باستخدام مفاتيح التعريب لتكون متوافقة مع اللغة الحالية
            ...
            const locationKey = card.querySelector('.text-primary').querySelector('small').getAttribute('data-key');

            const eventText = (
                translations[currentLang][titleKey] + ' ' +
                translations[currentLang][descKey] + ' ' +
                translations[currentLang][locationKey]
            ).toLowerCase();
        ...
        if (!resultsFound) {
            ...
            // تطبيق الترجمة على رسالة عدم العثور على نتائج
            noResultsMessage.textContent = translations[currentLang]['no_events_found'] || "No events found matching your criteria."; 
